...
module.exports = function(settings) {
  ...
  if (typeof settings.workingDirectory !== "string") {
    throw new Error("settings.workingDirectory is required. See https://github.com/scaramouche88/audiogram/blob/master/SERVER.md#all-settings for details.");
  }

  if (typeof settings.storagePath !== "string" && typeof settings.s3Bucket !== "string") {
    throw new Error("settings.storagePath or settings.s3Bucket is required. See https://github.com/scaramouche88/audiogram/blob/master/SERVER.md#all-settings for details.");
  }
  ...
  if ("maxUploadSize" in settings && typeof settings.maxUploadSize !== "number") {
    throw new TypeError("settings.maxUploadSize must be an integer. See https://github.com/scaramouche88/audiogram/blob/master/SERVER.md#all-settings for details.");
  }

  // Check fonts
  if ("fonts" in settings) {
    if (!Array.isArray(settings.fonts)) {
      throw new TypeError("settings.fonts must be an array of fonts. See https://github.com/scaramouche88/audiogram/blob/master/THEMES.md#a-note-about-fonts for details.")
    }

    settings.fonts.forEach(function(font){

      if (!font || typeof font.family !== "string" || typeof font.file !== "string") {
        return console.warn("Custom font in settings.fonts missing a 'family' or 'file'. See https://github.com/scaramouche88/audiogram/blob/master/THEMES.md#a-note-about-fonts for details.");
      }
